...
class Simulator:
    ...
    def find_effective_axis(self):
        ...
        for i in range(len(self.coordinates)):
            ...
            if joint_current:
                effective_rotations.append(possible_axi[i])
                block_index=np.argmax((self.coordinates[closest_stick_joint]-self.coordinates[i])!=0)
...
class Interface:
    ...
    def goal_test(self, state):
        ...
        if len(np.unique(axs[0]))-3 !=0 or \
            len(np.unique(axs[1]))-3!=0 or\
            len(np.unique(axs[2]))-3!=0 :
            return False
        return True
